...
# ╔═╡ 653904ea-65f6-4183-a5e3-4cb104c51a44
using Dates

# ╔═╡ 5d8e62ea-4065-4a99-a794-c072700ab70a
using Downloads: download

# ╔═╡ 57bb9ef7-5c3f-4733-853b-76891cd3944e
using DataFrames

# ╔═╡ 96a659c8-e81a-423e-a4ee-30cac2ddbf07
using Chain: @chain

# ╔═╡ 0282ebd6-08c5-4762-9c96-381165e8924d
using PlutoUI: TableOfContents, Slider

# ╔═╡ b4cc1913-1088-4020-8bfa-6025d0ff759d
using TimeZones

# ╔═╡ 52b4434e-1f45-48a3-b293-32928c1ab3d9
using DataFrameMacros

# ╔═╡ b8fa9acb-e07c-4075-ba4f-bab4570d6916
using CairoMakie, AlgebraOfGraphics

# ╔═╡ 5f956c39-a17f-42ab-8656-a9fa2d591759
using MeshViz: Viz

# ╔═╡ 1757dfb0-92f7-4154-ae88-7f39f51478fc
md"""
# Progress of Covid vaccinations in Austria
"""

# ╔═╡ e0897f84-0bc5-11ec-2cc1-d5e61e259b85
import CSV, HTTP, ZipFile

# ╔═╡ 3e774ae7-d162-492a-955d-ddfa6fee5111
md"""
# Getting the data
"""

# ╔═╡ e5ec6bee-f0f6-4f15-b074-99ee16b2f885
md"""
## Setting the time range
"""

# ╔═╡ c0fb8235-a340-4857-b995-9dd6f53b1732
start_date = Date(2021, 7, 28)

# ╔═╡ 9b37bf41-afd4-4530-81a7-c299739517d9
end_date = Date(now()) - Day(1)

# ╔═╡ 02e5a77f-54d2-41f4-bda3-3be52486be87
dates = start_date:Day(1):end_date

# ╔═╡ e3c49d2b-4d04-4be1-beae-c29c43442893
md"""
## Reading data for a given day
"""

# ╔═╡ 8bf003ce-1a00-42b7-b4fe-61fed8cb8667
function url(date)
	date_str = Dates.format(date, "yyyymmdd")
			"https://github.com/statistikat/coronaDAT/blob/master/archive/$(date_str)/data/$(date_str)_235959_impfdaten_orig_csv_bmsgpk.zip?raw=true"
end

# ╔═╡ e0cbe4a8-d31d-4b9c-8a69-1b8193e426dd
function load_data(date)
	zipfile = download(url(date))
	z = ZipFile.Reader(zipfile)
	zipped_file = only(z.files)
	unzipped_file = ZipFile.read(zipped_file)
	CSV.read(unzipped_file, DataFrame)
end

# ╔═╡ 08f46690-cef5-4ac0-aaff-b8091b7fbf7f
function clean_data(df)
	@chain df begin
		select(:Datum => parse_date, :Gemeindecode, :Bevölkerung, :Teilgeimpfte, :Vollimmunisierte, renamecols = false)
	end
end

# ╔═╡ 15952dc5-fe50-4520-a2d6-02ea13c35be0
function parse_date(dates)
	zdt_str = only(unique(dates))
	zdt = ZonedDateTime(zdt_str, "yyyy-mm-ddTHH:MM:SSzzzz")
	fill(Date(zdt), length(dates))
end

# ╔═╡ cc333555-e948-4ee8-83d8-19b22b50c7b4
date = first(dates)

# ╔═╡ 92881e1e-6493-4a2c-97fe-1c15d4fbed7f
@chain date begin
	load_data
	clean_data
end

# ╔═╡ 7a011328-635c-4571-a149-7294a79f8475
md"""
## Reading the data for all days
"""

# ╔═╡ 362add62-6ba2-4394-9cab-59a1ee151249
df0 = mapreduce(vcat, dates) do date
	@chain date begin
		load_data
		clean_data
	end
...
# ╔═╡ 258c59b2-43c6-47fd-b44b-3d35f07b9f37
md"""
# Visualizing the data
"""

# ╔═╡ 85ade8e9-033f-47a6-b01b-c9606027b756
md"""
## _Bundesländer_ and _Bezirke_ over time
"""

# ╔═╡ e73f55d0-bc04-4438-8243-428855d63ee2
md"""
## _Gemeinden_ over time
"""

# ╔═╡ 33dfa1a5-b8c4-4d2b-854a-925587164671
md"""
## Maps
"""

# ╔═╡ b67d8829-c81a-4ecc-9992-636e72400b28
begin
	import Meshes
...
# ╔═╡ 6a8cd190-eebe-4f74-b47a-0aa30ba88115
import GeoTables

# ╔═╡ ca31434b-3401-4492-8017-945323279aff
provinces = ["Burgenland", "Kärnten", "Niederösterreich", "Oberösterreich", "Salzburg", "Steiermark", "Tirol", "Vorarlberg", "Wien"]

# ╔═╡ 57596380-08fa-4057-98c5-5b95ffa3382d
df_aut = let
	df = mapreduce(vcat, provinces) do province
		t = GeoTables.gadm("AUT", province, children=true)
		DataFrame(t)
	end
	
	df.color = rand(size(df, 1))
	df
end

# ╔═╡ 3848614d-ce6e-4820-a771-a610b94b022f
data(df_aut) * visual(Viz, decimation = 0.001) * mapping(:geometry, elementcolor = :color) |> draw

# ╔═╡ fdab133b-1f3e-44ad-83d5-d9cac324710a
md"""
## Change over time
"""

# ╔═╡ f2d4359c-a8ea-46d8-90c9-a531186e9789
md"""
# Appendix
"""

# ╔═╡ 5f762be2-3dc2-4d7e-a46c-759b941af7b4
md"""
## _Gemeinden_, _Bezirke_, _Bundesländer_
"""

# ╔═╡ e0161078-b1a2-4209-b7ad-5c6d105ad903
gem_url = "http://www.statistik.at/verzeichnis/reglisten/gemliste_knz.csv"

# ╔═╡ e1cc1bf8-34c9-49c1-8bcb-392a93499b11
bez_url = "http://www.statistik.at/verzeichnis/reglisten/polbezirke.csv"

# ╔═╡ 0715828f-ad83-4b31-b309-c776fa54cc3d
function csv_from_url(url; kwargs...)
	CSV.File(HTTP.get(url).body; kwargs...) |> DataFrame
end

# ╔═╡ 6e5d6ed6-5a51-4bca-976b-6428426b89e2
bez_df = @chain bez_url begin
	CSV.File(HTTP.get(_).body, footerskip = 1, header = 3)
	DataFrame
	select!(:Bundesland, "Politischer Bezirk" => :Bezirk, "Politischer Bez. Code" => :BezCode)
end

# ╔═╡ 6df5b156-2820-4c27-bcd5-e87fb23945a8
gemeinden = @chain gem_url begin
	csv_from_url(footerskip = 1, header = 3)
	select(
		:Gemeindecode,
		:Gemeindecode => ByRow(x -> div(x, 100)) => :BezCode,
		:Gemeindename
		)
	leftjoin(_, bez_df, on = :BezCode)	
	select(Not(:BezCode))
end

# ╔═╡ 69febbdf-1760-4d6e-ba20-fb05bb4a1c64
df = @chain df0 begin
	leftjoin(gemeinden, on = :Gemeindecode)
end

# ╔═╡ d56eb0a1-2b26-4edc-8f46-354982feec09
@chain df begin
	groupby([:Datum, :Bezirk, :Bundesland])
	combine([:Bevölkerung, :Teilgeimpfte, :Vollimmunisierte] .=> sum, renamecols = false)
	@transform! begin
		:teil = :Teilgeimpfte / :Bevölkerung
		:voll = :Vollimmunisierte / :Bevölkerung
	end
	data
	_ * visual(Lines) * mapping(:Datum, :teil, layout = :Bundesland, group = :Bezirk)
	draw
end

# ╔═╡ 26bfa2ce-6e46-4237-a463-b1ccdfa6ef4a
@chain df begin
	...
	data
	_ * visual(Lines) * mapping(:Datum, [:teil, :voll], row = dims(1), color = :Bundesland)
	draw
end

# ╔═╡ 347cff5b-3f97-4da0-9db9-c425e3b66f0f
@chain df begin
	@transform(:x = :Teilgeimpfte / :Bevölkerung)
	data
	_ * mapping(:Datum, :x, group = :Gemeindecode => string, layout = :Bundesland) * visual(Lines, color = (:black, 0.1))
	draw
end

# ╔═╡ 6a2b6c07-653c-47c2-91c0-1a237352fe70
@chain df begin
	groupby([:Datum, :Bezirk, :Bundesland])
	combine([:Bevölkerung, :Teilgeimpfte, :Vollimmunisierte] .=> sum, renamecols = false)
	@transform! begin
	...
	stack([:teil, :voll], [:Bundesland, :Bezirk, :Datum])
	@transform! :Datum_lagged = :Datum - Day(14)
	leftjoin(_, select(_, Not(:Datum_lagged)), on = [:Datum_lagged => :Datum, :Bundesland, :Bezirk, :variable], makeunique = true)
	@transform! :change = :value - :value_1
	filter!(:change => !ismissing, _)
	data
	_ * mapping(:Datum, :change, group = :Bezirk, color = :Bundesland, row = :variable) * visual(Lines)
	draw
end

# ╔═╡ da20e5f1-f8d4-4a17-a729-2255dd67e8f0
@chain df begin
	@aside grp = [:Datum] #, :Bundesland]
	groupby(grp)
	combine([:Bevölkerung, :Teilgeimpfte, :Vollimmunisierte] .=> sum, renamecols = false)
	@transform! begin
		:teil = :Teilgeimpfte / :Bevölkerung
	...
	stack([:teil, :voll], grp)
	@transform! :Datum_lagged = :Datum - Day(7)
	leftjoin(_, select(_, Not(:Datum_lagged)), on = [:Datum_lagged => :Datum, grp[2:end]..., :variable], makeunique = true)
	@transform! :change = :value - :value_1
	filter!(:change => !ismissing, _)
	filter!(:variable => ==("teil"), _)
	data
	_ * mapping(:Datum, :change,
		#color = :Bundesland,
		row = :variable) * visual(Lines)
	draw
end

# ╔═╡ b09b1f22-8ea3-48f3-9341-1870f2657cb7
md"""
## Other
"""

# ╔═╡ 51afdcc9-b38b-46e1-adb7-66f049a5c36a
TableOfContents()
...
git-tree-sha1 = "9320c434cc1f4b7ab5b9e66b84ab7e2e4bf5bfeb"
...
git-tree-sha1 = "ad4ce2903089818ef6d81819ac8a7a525fc3fafc"
...
git-tree-sha1 = "d94da5bf6bcf14904f35b2ff042969484f7e520e"
