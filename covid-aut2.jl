...
df_hosp = @chain url_hosp begin
	df_from_remote_csv
	@transform(:date = Date(:Meldedatum, dateformat"dd.mm.YYYY HH:MM:SS"))
	select(Not(:Meldedatum))
end

# ╔═╡ 23956e11-54ab-4be2-8d6d-1a1c44c46a18
df_cases = @chain url_cases begin
	df_from_remote_csv
...
end_df = @chain df begin
	@groupby(:variable, :Bundesland)
	@combine(:latest = maximum(:date))
end
