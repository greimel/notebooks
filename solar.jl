...
# ╔═╡ 49922205-8dc5-4e4e-95c6-1797bb25f8f9
...
# ╔═╡ e44c3d63-d58b-4c3a-8faa-fb2846b94d8a
@chain df2 begin
	@subset(:year ≥ 2024)
#	@groupby(:date, :month, :year)
	#@transform(:quarter = :month ÷ 4)
	...
	@combine(
		:lo = minimum(:marketprice) / 1000,
		:hi = maximum(:marketprice) / 1000,
		:mean = mean(:marketprice) / 1000,
#		#:date = Date.(first(:year), first(:month))
		)
	@transform(:lo = :lo - :mean, :hi = :hi - :mean, :spread = :hi - :lo)
	#stack([:lo, :hi, :spread], :date)
	#data(_) * mapping(:date, :value, color = :variable
		#color = :year => nonnumeric
	#) * visual(Lines)
	#draw
	...
	@transform(
		:below05 =        :spread ≤ 0.05,
		:above05 = 0.05 < :spread ≤ 0.10,
		:above10 = 0.10 < :spread ≤ 0.15,
		:above15 = 0.15 < :spread 
	)
	stack([:below05, :above05, :above10, :above15], :date)
	@groupby(:month = month(:date), :variable)
	@combine(:count = sum(:value))
	unstack(:variable, :count)
...
#	@groupby(:date, :month, :year)
	...
	data(_) * mapping(:month => Int, :price,
		#color = :year => nonnumeric
	) * visual(ScatterLines)
	draw
end
...
manifest_format = "2.0"
project_hash = "d69d1d2c4a8516ae0b388d6692f28a36f480cc28"
